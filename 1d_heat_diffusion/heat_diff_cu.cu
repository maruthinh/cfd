...
static char help[] = "Solves the 1D heat diffusion equation.\n \
...
// q=1.0e6 ! heat flux (W/m^3)
  ...
  if(rank==0){
    ...
    PetscCall(VecCUDAGetArrayRead(vout, &soln));
    //PetscCall(VecGetArrayAndMemType(vout, &soln));
    /*for(int i=0; i<Nx; i++){
      std::cout << soln[i] << "\n";
    }*/
  }

  PetscCall(VecDestroy(&vout));
